(() => {
    ...
    class Popup {
        ...
        close(selectorValue) {
            ...
            const videoIframe = this.previousOpen.element.querySelector("iframe");
            if (videoIframe) {
                const originalSrc = videoIframe.getAttribute("src");
                videoIframe.removeAttribute("src");
                setTimeout((() => {
                    if (originalSrc) {
                        const pausedSrc = originalSrc.replace("autoplay=1", "autoplay=0");
                        videoIframe.setAttribute("src", pausedSrc);
                    }
                }), 100);
            }
    ...
    function initSliders() {
        if (document.querySelector(".slider-hero-page")) new swiper_core_Swiper(".slider-hero-page", {
            ...
            pagination: {
                el: ".slider-hero-page__pagination",
                clickable: true,
                dynamicBullets: true,
                dynamicMainBullets: 3
            },
            navigation: {
                prevEl: ".slider-hero-page__prev",
                nextEl: ".slider-hero-page__next"
            },
            on: {}
        });
    }
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        var elements = document.querySelectorAll(".video img, .video svg");
        elements.forEach((function(element) {
            element.addEventListener("click", (function() {
                var parent = element.closest(".video");
                var iframe = parent.querySelector("iframe");
                iframe.style.zIndex = "1";
                iframe.src = iframe.getAttribute("data-src") + "?rel=0&showinfo=0&autoplay=1";
            }));
        }));
        var items = document.querySelectorAll(".payment-checkout__item");
        function handleSelection() {
            document.querySelectorAll('.payment-checkout__item input[type="radio"]').forEach((function(radio) {
                radio.checked = false;
                radio.closest(".payment-checkout__item").classList.remove("selected");
            }));
        ...
        var defaultChecked = document.querySelector('.payment-checkout__item input[type="radio"]:checked');
        if (defaultChecked) {
            var defaultItem = defaultChecked.closest(".payment-checkout__item");
